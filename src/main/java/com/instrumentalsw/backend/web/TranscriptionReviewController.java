...
public final class TranscriptionReviewController {
    private final GetTranscriptionReview getReview;

    public TranscriptionReviewController(GetTranscriptionReview getReview) {
        this.getReview = getReview;
    }

    @GetMapping(value = "/{jobId}/review", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<TranscriptionReviewResponse> get(@PathVariable String jobId) {
        UUID parsed;
        try {
